...
public class RegMember1 extends JFrame {
	...
	JCheckBox[] hobbys;
	String hobby_collector = "no state";
	...
	private ItemListener handler = new ItemListener() {
		public void itemStateChanged(ItemEvent e) {
			if (e.getStateChange() == ItemEvent.SELECTED) {
				if (e.getItem() == hobbys[0]) {
					hobby_collector = hobby_collector + hobbys[0];
				} else if (e.getItem() == hobbys[1]) {
					hobby_collector = hobby_collector.concat(hobbys[1].toString());
				} else if (e.getItem() == hobbys[2]) {
					hobby_collector.concat(hobbys[2].toString());
				} else if (e.getItem() == hobbys[3]) {
					hobby_collector.concat(hobbys[3].toString());
				}
			}
		}
